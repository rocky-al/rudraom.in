...
{   
     ...
     public function orderDetails(Request $request)
     {
        ...
        if ($validator->fails()) {
         ...
         if($check_data > 0){  
            ...
            $itemval = Order::select('orders.id','orders.item_name','shipping_id','item_id','item_description','orders.item_price','orders.quantity','shipping_charges','order_status','reject_reason','orders.created_at','order_shipped_date','order_delivered_date','order_cancel_date','order_confirmed_date','address','apt_no','city','zip_code','country')     
            ->leftJoin('business_items', function($join) {
                $join->on('business_items.id', '=', 'orders.item_id');
            })        
    ...
    public function createCardToken(Request $request){
        ...
        if ($validator->fails()) {
            ...
            $token_result = PaymentController::createCardToken($request->number,$request->exp_month,$request->exp_year,$request->cvc);
            ...
            if($token_result){
                if(isset($user->stripe_customer_id) && !empty($user->stripe_customer_id)){
                    ...
                    Staff::where('id', $user->id)->update(['stripe_customer_id'=>$stripe_customer_id]);
                }
