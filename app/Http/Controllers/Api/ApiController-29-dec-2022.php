    ...
    public function businessList(Request $request)
      {
        ...
        if(isset($request->latitude) && !empty($request->latitude) && isset($request->longitude) && !empty($request->longitude) ){
            $query_total = "SELECT * FROM (SELECT categories.name as category_name,businesses.id,businesses.name,address,profile_img,ROUND(( $setDistance * acos( cos( radians ($request->latitude) ) * cos( radians(latitude) ) * cos( radians(longitude ) - radians($request->longitude ) ) + sin( radians($request->latitude) ) * sin( radians(latitude ) ) ) ),2) AS distance FROM businesses left join categories on businesses.category_id = categories.id    WHERE businesses.status='1' AND  businesses.is_delete='0' $filter_condition  HAVING distance < $location_radius  ORDER BY distance ASC) AS t";

        }else{
            $query_total = "SELECT * FROM (SELECT categories.name as category_name, businesses.id,businesses.name,address,profile_img, 10 AS distance FROM businesses left join categories on businesses.category_id = categories.id   WHERE businesses.status='1' AND  businesses.is_delete='0' $filter_condition HAVING distance < $location_radius   ORDER BY name ASC) AS t";

        } 
        $total_rows = DB::select(DB::raw( $query_total));
...
//HAVING distance < $location_radius
        ...
        if(isset($request->latitude) && !empty($request->latitude) && isset($request->longitude) && !empty($request->longitude) ){
            ...
            $query = "SELECT * FROM (SELECT categories.name as category_name, businesses.id,businesses.name,address,profile_img, 10 AS distance FROM businesses left join categories on businesses.category_id = categories.id   WHERE businesses.status='1' AND  businesses.is_delete='0' $filter_condition   ORDER BY name ASC ) AS t";
    ...
    public function businessDetails(Request $request)
       {
           ...
           $BusinessDetails->grade = $reviews_avg;
          ...
          foreach( $items_data as $itemval){
            ...
            $item_array[] = array(
                ...
                'item_img' => $item_image.'/'.$items_data[0]->image,
            );
          }
          ...
          $review_data = Reviews::select('name','image','business_review.id','rating','review','business_review.created_at')->leftJoin('users', function($join) {
              ...
              })->where('business_review.business_id',$request->business_id)->where('business_review.is_approved', '1')->orderBy('business_review.id', 'desc')->get();
     ...
     public function favouriteList(Request $request)
       {
           ...
           $review_data = Wishlist::select('businesses.id','name','profile_img','address')
                    ...
                    })->where('business_wishlist.user_id',$user->id)->orderBy('business_wishlist.id', 'desc')->get();
                    $bussiness_photo_url = url('uploads/business_image/');
          $data_array =  array();
              if($review_data ){
       ...
       // function for Add feedsList
     public function feedsList(Request $request)
     {
         $user = Auth::guard('api')->user();
         
         $review_data = Wishlist::select('businesses.id','name','profile_img','address','businesses.created_at')
                       ->leftJoin('businesses', function($join) {
                      $join->on('businesses.id', '=', 'business_wishlist.business_id');
                  })->where('business_wishlist.user_id',$user->id)->orderBy('business_wishlist.id', 'desc')->get();
            ...
            if($review_data ){
              foreach($review_data as $val){
                 ...
                 $busness_items = Business_item::select('id','item_name')->where('business_id', $val->id)->get();
                 ...
                 foreach( $busness_items as $itemval){
                    ...
                    $data_item_array[] = array(
                        "id" => $itemval->id,
                        "item_name" => $itemval->item_name, 
                        "item_img" =>  $bussiness_item_url.'/'.$items_img[0]->image,  
                       );
