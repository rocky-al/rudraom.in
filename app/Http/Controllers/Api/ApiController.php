...
{   
    ...
    public function businessList(Request $request)
      {
        ...
        //   }
         ...
         if(isset($request->user_id) && !empty($request->user_id)){
           $noti_data=Notification::where(['user_id'=>$request->user_id,'is_read'=>'0'])->count();
                if($noti_data>0){
                    $stat_noti=true;
                }
                else{
    ...
    public function addReview(Request $request)
    {
     ...
     public function favouriteList(Request $request)
       {
        ...
        if(isset($user->id) && !empty($user->id)){
           $noti_data=Notification::where(['user_id'=>$user->id,'is_read'=>'0'])->count();
                if($noti_data>0){
                    $stat_noti=true;
                }
                else{
     ...
     public function notificationList(Request $request)
     {
         ...
         if(count($notification_data )> 0){  
            ...
            $update_record = Notification::where(['user_id'=>$user->id,'notification_type'=>NULL])->update(['is_read'=>'1']);
  ...
  // by pranav 
    ...
    $update_record = Notification::where('id',$request->notification_id)->update(['is_read'=>'1']);
    ...
    //    }
     ...
     public function orderDetails(Request $request)
     {
        ...
        if ($validator->fails()) {
         ...
         if($check_data > 0){ 
             $raisequery_order_count = Query::select('*')->where('order_id', $request->id)->count();
                if($raisequery_order_count > 0){
                $raisequery_order_date = Query::select('id','order_id','created_at')->where('order_id', $request->id)->orderByDesc('id')->first();
               $raise_query_date= date("d-m-Y H:i:s", strtotime($raisequery_order_date->created_at));
                }
                else{
                    $raise_query_date='';
                }
            
            $itemval = Order::select('orders.tracking_no','businesses.name','orders.id','orders.item_name','shipping_id','item_id','item_description','orders.item_price','orders.quantity','shipping_charges','order_status','orders.reject_reason','orders.created_at','order_shipped_date','order_delivered_date','order_cancel_date','order_confirmed_date','orders.address','apt_no','city','zip_code','country')     
            ->leftJoin('business_items', function($join) {
                $join->on('business_items.id', '=', 'orders.item_id');
            },
            )    
            ...
            $items_img = DB::select(DB::raw( $items_qeury_img));
               ...
               $data_item_array = array(
                   "id" => $itemval->id,
                   "tracking_no"=>$itemval->tracking_no,
                   "item_name" => $itemval->item_name,
                   ...
                   "item_img" =>  $items_img,  
                   "raise_query_date" => $raise_query_date,
                  );
     ...
     public function cancelOrder(Request $request)
     {
        ...
        if ($validator->fails()) {
         ...
         if($check_data > 0){  
            ...
            /*******Email send to Business **********/

            $user_data=Business::find($order->business_id);
            ...
            $replace_data = [ 
                 ...
                 sendMail($user_data->email_address, $emailval, $subject);

            //Refund Added By  Pranav 

            $refund_result = PaymentController::refundOrder($order->transaction_id);
      ...
      public function raiseQuery(Request $request)
     {
        ...
        if ($validator->fails()) {
            ...
            $data->message = $request->message;
             ...
             if($data->save()){
                return response()->json([ 'status_code' => 200,'status' => true, 'message' => 'Query added successfully.'], 200);
            }else{
                return response()->json([ 'status_code' => 400,'status' => false, 'message' => 'Something went wrong.'], 200);
            }
