    ...
    public function resendOTP(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if ($this->otp_email( $email, $otp, '')) {  // if otp sended to user email for account verification
                return $this->sendResponse($otp_array, 'OTP send successfully.');
            } else {
    ...
    public function businessList(Request $request)
      {
        ...
        if(isset($request->name) && !empty($request->name)){
            $filter_condition = " and name like '%".$request->name."%'";
        }
    ...
    public function businessDetails(Request $request)
       {
          ...
          foreach( $review_data as $reviewVal){ 
            $review_array[] = array(
                ...
                'created_at' => time_elapsed_string($reviewVal->created_at),
    ...
    // function for Add favourite
          ...
          $data_array =  array();
              if($review_data ){
                foreach($review_data as $val){
                   ...
                   // $reviews_avg = Reviews::select('id')->where('business_id', $request->business_id)->where('user_id', $user->id)->count();
     ...
     public function feedsList(Request $request)
     {
         ...
         $review_data = Wishlist::select('businesses.id','name','profile_img','address','businesses.created_at')
            ...
            $bussiness_photo_url = url('uploads/business_image/');
            $bussiness_item_url = url('uploads/business_item_img/');
            $data_array =  array();
            if($review_data ){
              foreach($review_data as $val){
                 ...
                 foreach( $busness_items as $itemval){
                  ...
                  $data_array[] = array(
                       "id" => $val->id,
                       "name" => $val->name,
                       "created_at" => time_elapsed_string($val->created_at),
                       "bussiness_img" =>  $bussiness_photo_url.'/'.$val->profile_img,
