    ...
    public function businessList(Request $request)
      {
        ...
        if(isset($request->latitude) && !empty($request->latitude) && isset($request->longitude) && !empty($request->longitude) ){
            $query = "SELECT * FROM (SELECT categories.name as category_name,businesses.id,businesses.name,latitude,longitude,address,profile_img,ROUND(( $setDistance * acos( cos( radians ($request->latitude) ) * cos( radians(latitude) ) * cos( radians(longitude ) - radians($request->longitude ) ) + sin( radians($request->latitude) ) * sin( radians(latitude ) ) ) ),2) AS distance FROM businesses left join categories on businesses.category_id = categories.id    WHERE businesses.status='1' AND  businesses.is_delete='0' $filter_condition HAVING distance < $location_radius ORDER BY distance ASC LIMIT $offset, $no_of_records_per_page) AS t";

        }else{
            $query = "SELECT * FROM (SELECT categories.name as category_name, businesses.id,businesses.name,address,latitude,longitude,profile_img, 10 AS distance FROM businesses left join categories on businesses.category_id = categories.id   WHERE businesses.status='1' AND  businesses.is_delete='0' $filter_condition HAVING distance < $location_radius   ORDER BY name ASC LIMIT $offset, $no_of_records_per_page) AS t";
    ...
    public function businessDetails(Request $request)
       {
           ...
           $BusinessDetails->profile_img = url('uploads/business_profile_medium/').'/'.$BusinessDetails->profile_img;
  
          $bussiness_photo_url = url('uploads/business_image_small/');
          ...
          $item_image = url('uploads/item_image_small/');
    ...
    public function addReview(Request $request)
    {
     ...
     public function feedsList(Request $request)
     {
         ...
         $review_data = Wishlist::select('businesses.id','business_items.id as item_id','item_description','item_name','name','profile_img','address','business_items.created_at')
            ...
            $bussiness_photo_url = url('uploads/business_profile_small/');
            $bussiness_item_url = url('uploads/business_image_medium/');
            $data_array =  array();
            if($review_data ){
              foreach($review_data as $val){
     ...
     {         
            ...
            $itemval = Business_item::select('id','item_name','item_price','item_description')->where('id', $request->item_id)->first();
     ...
     {
         $user = Auth::guard('api')->user(); 
         $notification_data = Notification::select('id','title','message','notification_type','created_at')
     ...
     public function createOrder(Request $request)
     {
        ...
        // $get_carts = Carts::select('id')->where('user_id', $user->id)->get();
     ...
     public function orderDetails(Request $request)
     {
        ...
        if ($validator->fails()) {
         ...
         if($check_data > 0){  
            ...
            $itemval = Order::select('orders.id','orders.item_name','shipping_id','item_id','item_description','orders.item_price','orders.quantity','shipping_charges','order_status','reject_reason','orders.created_at','order_shipped_date','order_delivered_date','order_cancel_date','order_confirmed_date')     
            ->leftJoin('business_items', function($join) {
                $join->on('business_items.id', '=', 'orders.item_id');
            })        
