...
*/
        ...
        public function order_manage(Request $request)
    {
        ...
        if($request->get('status')==4 ){
         ...
         // Refund 
         $refund_result = PaymentController::refundOrder($data->transaction_id);
         }
        ...
        $data->tracking_no=$request->get('tracking_no');
        }
        ...
        $data->order_delivered_date = date("Y-m-d");
            ...
            if ($data->save()) {
                ...
                $status = 'Confirmed';
                $message=''.$value->name.' has confirmed your order for '.$data->item_name.'. Follow updates to your order status by clicking the “Status” button at the bottom of the home screen.';
                } 
                elseif ($data->order_status == 2) {
                $status = 'Shipped';

                $message='Your order for '.$data->item_name.' has been shipped by '.$value->name.'.';
                } 
                elseif ($data->order_status == 3) {
                $status = 'Delivered';

                $message='Hey '.$user_data->name.'! Your order for '.$data->item_name.' has been delivered by '.$value->name.'. Let us know if you did not receive it.';
                } 
                elseif ($data->order_status == 4) {
                $status ='Cancelled';
                 
                $message='Your order for '.$data->item_name.' was cancelled by '.$value->name.'.';
                }
     ...
     if($data->order_status == 5 || $data->order_status == 4 || $data->order_status == 3 || $data->order_status == 2 || $data->order_status == 1){
            send_notification_FCM($title='Order '.$status, $message, $token=$user_data->fcm_token, $redirection =$data->id,'order');
