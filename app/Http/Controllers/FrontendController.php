...
class FrontendController extends Controller
{
    ...
    {
        $folderPath = base_path('uploads/temp_crop/');
        //echo $folderPath;dd();
     ...
     public function register(Request $request)
    {
       $validator = Validator::make($request->all(), [
                'photos.*' => 'max:10200',
                'profife_pic'=>'max:10200',
               ]);
        if ($validator->fails()) { 
                  return response()->json(['status' => false, 'message' =>'Maximum 10 MB size of  Photos are allowed'], 200);
              }
...
/*            if (isset($request->profife_pic)) {
         ...
         if (isset($request->crop_img)) {
             
            $file_move=base_path('uploads/temp_crop/'.$request->crop_img.'');
            $dest=base_path('uploads/business_profile/'.$request->crop_img.'');
            $result_copy=copy($file_move, $dest);
            $path_medium="uploads/business_profile_medium/"; // medium image folder 
            $width_med=800;  // medium image width
            $height_med=800; // medium image height

            $path_small="uploads/business_profile_small/"; // medium image folder 
            $width_small=512;  // medium image width
            $height_small=512; // medium image height

            $image_resize = \Image::make($file_move);

            $image_resize->resize($width_med,$height_med, function ($constraint) {
            $constraint->aspectRatio();
            })->save($path_medium .$request->crop_img);
             
            $image_resize->resize($width_small,$height_small, function ($constraint) {
            $constraint->aspectRatio();
            })->save($path_small .$request->crop_img);

            if($result_copy){
              $data->profile_img =$request->crop_img;
            }
         // unlink img  
        unlink('uploads/temp_crop/'.$request->crop_img);
         }
