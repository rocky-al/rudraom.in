...
class HomeController extends Controller
{
    public function index()
    {
        ...
        $user_data_main =  User::where('id','!=' ,'1')->where('is_delete', '0');
        $user_data_main_active =  User::where('id','!=' ,'1')->where('status', '1')->where('is_delete', '0');
        $user_data_main_inactive =  User::where('id','!=' ,'1')->where('status', '0')->where('is_delete', '0');
       
        $user_count = $user_data_main->count();
                   $earning = 0;

        $user_count_active = $user_data_main_active->count();
        $user_count_inactive = $user_data_main_inactive->count();
