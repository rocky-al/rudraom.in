...
class CategoryController extends Controller
{
    ...
    /*    if(Category::where('name', '=', $request->get('name'))->count() > 0){
            ...
            if (isset($id) && !empty($id)) {
                $name_validate = Category::where('name', request('name'))->where('id', '!=', $request->id)->where('is_delete','0')->first();
            } else {
                $name_validate = Category::where('name', request('name'))->where('is_delete','0')->first();
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            foreach ($data as $value) {
                ...
                $row['updated_at'] = date('d M Y ', strtotime($value->updated_at));
