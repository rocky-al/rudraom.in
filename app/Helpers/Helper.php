...
function imageResize($image,$width='',$height='',$width1='',$height1='',$width2='',$height2='',$path='',$path1='',$path2='')
{
    $file = $image;
    $name = $image->getClientOriginalName();
 ...
 /*   $image_resize->resize($width,$height, function ($constraint) {
    ...
    $image_resize->resize($width1,$height1, function ($constraint) {
            $constraint->aspectRatio();
        })->save($path1 . $filename);

    $image_resize->resize($width2,$height2, function ($constraint) {
            $constraint->aspectRatio();
        })->save($path2 . $filename);
...
// send notification 
function send_notification_FCM($title='', $message='', $token='', $redirection = '',$notification_type='')
{
