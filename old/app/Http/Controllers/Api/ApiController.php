    ...
    public function updatePassword(Request $request)
    {
      ...
      public function businessList(Request $request)
      {
        ...
        if(isset($request->latitude) && !empty($request->latitude) && isset($request->longitude) && !empty($request->longitude) ){
            ...
            $query = "SELECT * FROM (SELECT id,name,address,profile_img, 10 AS distance FROM businesses   WHERE status='1' AND  is_delete='0' $filter_condition  ORDER BY name ASC) AS t";
